...
Три: **нова** → **зарахована** → **архів**. Проміжні «підтверджена» й
«відмова» прибрано — вони лише плодили роботу з їх проставляння. Якщо
такі статуси були в базі, при першому запуску нової версії вони самі
переїжджають: «підтверджена» → «нова», «відмова» → «архів» (у журнал
пишеться, скільки саме).

Змінити можна двома способами: у картці дитини списком, або одразу
кільком — кнопками **«Зарахувати»** і **«В архів»** у нижній панелі.
Обидві питають підтвердження: коли ввімкнено фільтр за класом, усі
рядки позначаються самі, і один клік торкнувся б цілого класу.

Дія зворотна, тому доступна обом ролям — і адміністратору, і педагогу.

### Друковані списки

Дві форми, обидві з кошика внизу списку заявок:

* **«Тільки прізвища й дати»** — аркуш для відмічання відвідування:
  прізвища й пʼять робочих днів. Книжковий формат.
  Поруч є галочка **«без дат»**: коли заняття переносяться і йдуть не
  поспіль, дати не друкуються — колонки лишаються порожніми, числа
  вписуються ручкою просто на аркуші. Вибір тижня при цьому згасає,
  бо вже ні на що не впливає.
* **«Повний список»** — аркуш для звірки на перших заняттях. Дат немає.
  Колонки залежать від напряму:
  * **ГПД** — прізвище, клас, дата народження, школа, батьки з телефонами,
    хто забирає дитину;
  * **НМТ** — прізвище, клас, телефон учня, школа, обрані предмети,
    батьки з телефонами.

**Один аркуш — один напрям.** Кошик набирає напрям від першого доданого учня
й далі приймає лише таких самих; про пропущених пише окремим рядком. Це не
примха: у ГПД і НМТ різні колонки, і спільний аркуш виходить нечитабельним.

Алергії, побажання батьків та інші поля анкети на аркуш не потрапляють —
це список для звірки, а не картка дитини.

### Класи й перехід на дванадцятирічку

Перелік класів у формі задається **однією цифрою** — `GRADE_MAX` у блоці
`CFG` на початку `form.html`. Щоб додати 12 клас, досить змінити `11` на `12`.

Решта системи вже це витримує: напрям рахується від класу (`program_for()`:
9 і вище — НМТ, нижче — ГПД), фільтр класів в адмінці будується з того, що
є в базі, а списки сортуються числом. Перевірено наскрізно — заявка 12 класу
проходить, потрапляє в НМТ і друкується правильно.
