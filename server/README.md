...
Кнопка внизу списку заявок, **тільки для ролі `admin`**. Надсилає на адресу
з `ADMIN_MAIL` два вкладення: таблицю з усіма даними (редагуйте як завгодно)
і точну резервну копію `.db`. Хеші паролів у таблицю навмисно не потрапляють.
Кожне натискання пишеться в журнал доступу (`audit`).
